...
NSString * MIU1401_ORorke_Melanie_WeekFour$ModuleRequireFormat = @"(function(exports){"
...
@implementation MIU1401_ORorke_Melanie_WeekFourObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_miu1401_ororke_melanie_weekfour);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_miu1401_ororke_melanie_weekfour gc];
}
...
{
	// create MIU1401_ORorke_Melanie_WeekFour global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "MIU1401_ORorke_Melanie_WeekFour" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_miu1401_ororke_melanie_weekfour = [[MIU1401_ORorke_Melanie_WeekFourObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_miu1401_ororke_melanie_weekfour];
	
	NSString *_miu1401_ororke_melanie_weekfourNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _miu1401_ororke_melanie_weekfourNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_miu1401_ororke_melanie_weekfour valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:MIU1401_ORorke_Melanie_WeekFour$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.miu1401_ororke_melanie_weekfour.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.miu1401_ororke_melanie_weekfour.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
