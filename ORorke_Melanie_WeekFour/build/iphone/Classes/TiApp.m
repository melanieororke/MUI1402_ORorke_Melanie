...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kMIU1401_ORorke_Melanie_WeekFourUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kMIU1401_ORorke_Melanie_WeekFourUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a MIU1401_ORorke_Melanie_WeekFour app using the node.js CLI
